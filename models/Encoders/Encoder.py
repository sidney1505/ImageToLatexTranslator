...
class Encoder:
    ...
    def encodeRowsBidirectionalStacked(self):
        with tf.variable_scope(self.model.encoder + '_rows', reuse=None):
            shape = tf.shape(self.model.features)
            batchsize = shape[0]
            height = shape[1]
            ...
            self.refined_rows = tf.reshape(rowfeatures, [batchsize,height,width,\
                2 * self.channels]) # (20, 4, 48, 2048)
            rowstates = tf.concat([rowstates[0],rowstates[1]],-1)
            nrowstates = []
            w1 = tf.get_variable('weight', [4 * self.channels, 2 * self.channels], \
                    tf.float32, tf.random_normal_initializer())
            b1 = tf.get_variable('bias', [2 * self.channels], tf.float32, \
            ...
            # code.interact(local=dict(globals(), **locals()))
            for i in range(self.model.number_of_layers):
                s = tf.concat([rowstates[i][0],rowstates[i][1]],-1)
                s = tf.tensordot(s,w1,[[-1],[0]]) + b1
                s = tf.nn.tanh(s)
                s = tf.nn.dropout(s,self.model.keep_prob)
                nrowstates.append(s)
            rowstates = tf.concat(nrowstates,0)
            rowstates = tf.reshape(rowstates, \
                [self.model.number_of_layers * batchsize,height,2*self.channels])
            # code.interact(local=dict(globals(), **locals()))
        ...
        with tf.variable_scope(self.model.encoder + '_rows2', reuse=None):
            ...
            print('hi')
            # code.interact(local=dict(globals(), **locals()))
            row_summary = tf.reshape(rowstates,
                [2,
                self.model.number_of_layers,
                batchsize,                
                2 * self.channels])
            self.row_summary = tf.transpose(row_summary, [1,0,2,3]) 

    def encodeColsBidirectionalStacked(self):
        with tf.variable_scope(self.model.encoder + '_cols', reuse=None):
            shape = tf.shape(self.model.features)
            batchsize = shape[0]
            height = shape[1]
            ...
            def createMulticell():
                ...
                for i in range(self.model.number_of_layers):
                    cells.append(tf.contrib.rnn.BasicLSTMCell(self.channels))
                    initial_states.append(initial_state_cols)
                initial_states = tuple(initial_states)
                multicell = tf.contrib.rnn.MultiRNNCell(cells)
                return multicell, initial_states
            colrnncell_fw, initial_state_cols = createMulticell()
            colrnncell_bw, _ = createMulticell()
            colfeatures, colstates = tf.nn.bidirectional_dynamic_rnn( \
                colrnncell_fw, \
                colrnncell_bw, \
                colfeatures, \
                initial_state_fw=initial_state_cols, \
                initial_state_bw=initial_state_cols, \
                parallel_iterations=1)
            colfeatures = tf.concat([colfeatures[0],colfeatures[1]],-1)
            colfeatures = tf.reshape(colfeatures, [batchsize,width,height,\
                2 * self.channels]) # (20, 4, 48, 2048)
            self.refined_cols = tf.transpose(colfeatures, [0,2,1,3])
            colstates = tf.concat([colstates[0],colstates[1]],-1)
            ncolstates = []
            w1 = tf.get_variable('weight', [4 * self.channels, 2 * self.channels], \
                    tf.float32, tf.random_normal_initializer())
            b1 = tf.get_variable('bias', [2 * self.channels], tf.float32, \
                    tf.random_normal_initializer())
            for i in range(self.model.number_of_layers):
                s = tf.concat([colstates[i][0],colstates[i][1]],-1)
                s = tf.tensordot(s,w1,[[-1],[0]]) + b1
                s = tf.nn.tanh(s)
                s = tf.nn.dropout(s,self.model.keep_prob)
                ncolstates.append(s)
            colstates = tf.concat(ncolstates,0)
            colstates = tf.reshape(colstates, \
                [self.model.number_of_layers * batchsize, width , 2 * self.channels])
            # code.interact(local=dict(globals(), **locals()))
            ...
            with tf.variable_scope(self.model.encoder + '_cols2', reuse=None):
                ...
                _, colstates = tf.nn.dynamic_rnn(colstaternncell, colstates, \
                    dtype=tf.float32, parallel_iterations=1)
                col_summary = tf.reshape(colstates, \
                    [2, \
                    self.model.number_of_layers, \
                    batchsize, \
                    2 * self.channels])
                self.col_summary = tf.transpose(col_summary, [1,0,2,3])