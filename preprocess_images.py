...
def process_args(args):
    parser = argparse.ArgumentParser(description='Process images for ease of training. \
        Crop images to get rid of the background. For a cropped image of size (w,h), \
        we pad it with PAD_TOP, PAD_BOTTOM, PAD_LEFT, PAD_RIGHT, and the result is of \
        size (w+PAD_LEFT+PAD_RIGHT, h+PAD_TOP+PAD_BOTTOM. Then we see which bucket it \
        falls into and pad them with whitespace to match the smallest bucket that can \
        hold it. Finally, downsample images.')
    ...
    parser.add_argument('--crop-blank-default-size', dest='crop_blank_default_size',
                        ...
                        help=('If an image is blank, this is the size of the cropped \
                            image, should be a Json string. Default=(600,60).'
                        ))
    parser.add_argument('--pad-size', dest='pad_size',
                        type=str, default='[8,8,8,8]',
                        help=('We pad the cropped image to the top, left, bottom, \
                            right with whitespace of size PAD_TOP, PAD_LEFT, PAD_BOTTOM, \
                            PAD_RIGHT, should be a Json string. Default=(8,8,8,8).'
                        ))
    parser.add_argument('--buckets', dest='buckets',
                        type=str, default='[[240, 100], [320, 80], [400, 80], [400, 100], \
                            [480, 80], [480, 100], [560, 80], [560, 100], [640, 80], \
                            [640, 100], [720, 80], [720, 100], [720, 120], [720, 200], \
                            [800, 100], [800, 320], [1000, 200], [1000, 400], [1200, 200], \
                            [1600, 200], [1600, 1600]]',
                        help=('Bucket sizes used for grouping. Should be a Json string. \
                            Note that this denotes the bucket size after padding and \
                            before downsampling.'
                        ))
...
def main_parallel(l):
    filename, postfix, output_filename, crop_blank_default_size, pad_size, buckets, \
    downsample_ratio = l
